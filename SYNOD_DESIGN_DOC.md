...
*   **Execution:** Code runs in a child process forked from a `forkserver` (with pre-loaded matplotlib, pandas, numpy) as a non-root user with restricted builtins (no exec, eval, compile, open) and a 60-second timeout. The forkserver avoids the overhead of re-importing heavy libraries on every execution. Exceptions are caught and fed back to the LLM for self-debugging.
